...
public final class EditStringListScreen extends Screen
{
	private final Screen prevScreen;
	private final StringListSetting stringList;
	
	private ListGui listGui;
	private TextFieldWidget stringNameField;
	...
	public EditStringListScreen(Screen prevScreen, StringListSetting stringList)
	{
	...
	public void init()
	{
		listGui = new ListGui(client, this, stringList.getValues());
		
		stringNameField = new TextFieldWidget(client.textRenderer,
			width / 2 - 152, height - 55, 150, 18, new LiteralText(""));
		children.add(stringNameField);
	...
	public boolean mouseClicked(double mouseX, double mouseY, int mouseButton)
	{
		...
		stringNameField.mouseClicked(mouseX, mouseY, mouseButton);
	...
	public void tick()
	{
		stringNameField.tick();
		
		stringToAdd = stringNameField.getText().toLowerCase();
		addButton.active = !stringToAdd.isEmpty();
		...
		float partialTicks)
	{
		...
		drawCenteredString(matrixStack, client.textRenderer,
			stringList.getName() + " (" + listGui.getItemCount() + ")",
			width / 2, 12, 0xffffff);
		
		stringNameField.render(matrixStack, mouseX, mouseY, partialTicks);
		...
		if(stringNameField.getText().isEmpty() && !stringNameField.isFocused())
			drawStringWithShadow(matrixStack, client.textRenderer,
				"String", 68, height - 50, 0x808080);
		...
		listGui.renderIconAndGetName(matrixStack, stringToAdd, 52,
			height - 52, false);
	...
	private static class ListGui extends ListWidget
	{
		...
		public ListGui(MinecraftClient mc, EditStringListScreen screen,
			List<String> list)
		{
			...
			int y, int var4, int var5, int var6, float partialTicks)
		{
			...
			fr.draw(matrixStack, string, x, y, 0xf0f0f0);
		}
		
		private String renderIconAndGetName(MatrixStack matrixStack,
			String string, int x, int y, boolean large)
		{
			if(string == null || string.isEmpty())
			{
				GL11.glPushMatrix();
				...
				mc.getItemRenderer().renderInGuiWithOverrides(
					new ItemStack(Items.BARRIER), 0, 0);
			...
			}else
			{
				...
				DiffuseLighting.enable();
				mc.getItemRenderer().renderInGuiWithOverrides(
						new ItemStack(Blocks.GREEN_CONCRETE), 0, 0);
				DiffuseLighting.disable();
